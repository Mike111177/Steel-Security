...
public class Config {
	...
	private void defaults() { // Defaults
		...
		plugin.getConfig().addDefault("General.Logon_Message",
				"Steel Security is running on this server.");
		...
		plugin.getConfig().addDefault("AntiSpam.Censoring.Block_Words",
				Arrays.asList("Nodus", "Avo", "ICHG"));
		plugin.getConfig().addDefault("AntiSpam.Censoring.Canceling.Enabled",
				false);
		plugin.getConfig().addDefault(
				"AntiSpam.Censoring.Canceling.Minimum_Length", 4);
		plugin.getConfig().addDefault("AntiSpam.Censoring.Canceling.Percent",
				25);
		...
		plugin.getConfig()
				.addDefault("Offline_GameMode_Changer.Enabled", false);
		plugin.getConfig().addDefault(
				"Offline_GameMode_Changer.Default_GameMode", 0);
		plugin.getConfig().addDefault("Block_Blacklist.Enabled", false);
		plugin.getConfig().addDefault("Block_Blacklist.Blocks",
