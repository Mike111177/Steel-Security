...
public class Sts extends SSCmdExe {
		...
		if (args.length == 0) {
			if (sender.hasPermission("steelsecurity.commands.sts")) {
				...
				sender.sendMessage(g
						+ "For a list of commands please type /sts help");
			} else {
			...
			if (args[0].equalsIgnoreCase("checkperm")) {
				if (sender.hasPermission("steelsecurity.commands.checkperm")) {
					if (args.length < 3) {
						...
						String perm = args[2];
						String yes = (g + target.getName()
								+ " has the permission " + perm);
						String no = (r + target.getName()
								+ " does not have the permission " + perm);
						String world = "";
						String worldname = "";
						if (args.length == 4) {
						...
						if (target.isOnline()) {
							if (args.length == 3) {
								...
								if (pex.has(target.getName(), perm, world)) {
									sender.sendMessage(yes + " in world "
											+ worldname);
								} else {
									sender.sendMessage(no + " in world "
											+ worldname);
								}
							}
						} else {
							if (Bukkit.getServer().getPluginManager()
								...
								if (args.length == 4) {
									...
									sender.sendMessage(r
											+ "Please define a a world when checking a permission of an offline player.");
								}
							} else {
								sender.sendMessage(r
										+ "Please install PermissionsEx to use this command with references to offline players.");
							}
						}
					}
				} else {
			...
			if (args[0].equalsIgnoreCase("listop")) {
				if (sender.hasPermission("steelsecurity.commands.listop")) {
					...
					int total = 0;
					int online = 0;
					for (OfflinePlayer player : Bukkit.getServer()
							.getOfflinePlayers()) {
						if (player.isOp()) {
							total = total + 1;
							if (player.isOnline()) {
					...
					if (list.equals("")) {
						sender.sendMessage(r
								+ "Sorry, there appears to be no ops on this server!");
					} else {
						sender.sendMessage(w + "There are " + b + online + w
								+ " out of " + b + total + w + " ops online.");
			...
			if (args[0].equalsIgnoreCase("gamemode")) {
				if (sender.hasPermission("steelsecurity.commands.gamemode")) {
					new GameModeCmdCatch(noperm, plugin).stsgamemode(sender,
							args);
				} else {
					sender.sendMessage(noperm);
				}
			...
			if (args[0].equalsIgnoreCase("checkgm")) {
				if (sender.hasPermission("steelsecurity.commands.checkgm")) {
					if (args.length == 2) {
						String target = Bukkit.getServer().getPlayer(args[2])
								.getName();
						FileConfiguration config = PlayerConfigManager
								.getConfig(target);
						int gm = config.getInt("GameMode");
						sender.sendMessage(target + " is in"
								+ GameMode.getByValue(gm).name() + "mode.");
					}
				}
			}
			if (args[0].equalsIgnoreCase("reload")) {
				if (sender.hasPermission("steelsecurity.commands.reload")) {
					plugin.reloadConfig();
					sender.sendMessage(g
							+ plugin.getConfig().getString("General.Prefix")
							+ ": Config Reloaded.");
				} else {
					sender.sendMessage(noperm);
				}
	...
	private void p1(CommandSender sender) {
		...
		if (sender.hasPermission("steelsecurity.commands.help")) {
			sender.sendMessage(g + "/sts help:" + y
					+ " Displays this help screen.");
		}
		if (sender.hasPermission("steelsecurity.commands.checkperm")) {
			sender.sendMessage(g + "/sts checkperm:" + y
					+ " Checks a permmision for another player.");
		}
		if (sender.hasPermission("steelsecurity.commands.listop")) {
			sender.sendMessage(g + "/sts listop:" + y + " List ops.");
		}
		if (sender.hasPermission("steelsecurity.commands.gamemode")) {
			sender.sendMessage(g + "/sts gamemode:" + y
					+ " Changes the gamemode of another player.");
		}
		if (sender.hasPermission("steelsecurity.commands.checkgm")) {
			sender.sendMessage(g + "/sts checkgm:" + y
					+ " Checks what Game Mode a player is in.");
		}
		if (sender.hasPermission("steelsecurity.commands.reload")) {
