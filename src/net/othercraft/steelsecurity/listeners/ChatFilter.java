		...
		try {
			...
			if (check > speed
					|| !plugin.getConfig().getBoolean(
							"AntiSpam.AntiFlood.Enabled")
					|| !event.getPlayer().hasPermission(
							"steelsecurity.bypass.antiflood")) {// checks if the
																// speed of chat
																// is faster
																// than what is
																// configured
			...
			if ((!spam)) {// only bothers with the anticaps and the censoring if
							// it isnt spam.
				if (plugin.getConfig().getBoolean("AntiSpam.Censoring.Enabled")
						&& !event.getPlayer().hasPermission(
								"steelsecurity.bypass.censor")) {// checks if it
																	// so scan
																	// for
																	// configured
																	// word
					@SuppressWarnings("unchecked")
					List<String> list = (List<String>) plugin.getConfig()
							.getList("AntiSpam.Censoring.Block_Words");// retreives
																		// the
																		// list
																		// of
																		// blocked
				...
				if (event.getMessage().length() > plugin.getConfig().getInt(
						"AntiSpam.Censoring.Canceling.Minimum_Length")) {//
					if (plugin.getConfig().getBoolean(
							"AntiSpam.Censoring.Canceling.Enabled")
							&& !event.getPlayer().hasPermission(
									"steelsecurity.bypass.censor")) {// checks
																		// for
																		// if it
																		// should
																		// do
																		...
																		// anticaps
																		// check
						double percent = plugin.getConfig().getInt(
								"AntiSpam.Censoring.Canceling.Percent");// gets
																		// the
																		// configured
																		// percent
						...
						while (capcounter < capcount) {
							if (message.toCharArray()[capcounter] != ("*")
									.toCharArray()[1]) {// counts both upper
														// case and lower case
														// letters
								++lowercase;
							} else {
				...
				if (event.getMessage().length() > plugin.getConfig().getInt(
						"AntiSpam.AntiCaps.Minimum_Length")) {//
					if (plugin.getConfig().getBoolean(
							"AntiSpam.AntiCaps.Enabled")
							&& !event.getPlayer().hasPermission(
									"steelsecurity.bypass.anticaps")) {// checks
																		// for
																		// if it
																		// should
																		// do
																		...
																		// check
						double percent = plugin.getConfig().getInt(
								"AntiSpam.AntiCaps.Percent");// gets the
																// configured
																// percent
						int capcount = message.length();
						int capcounter = 0;
						...
						Double lowercase = 0.0;
						while (capcounter < capcount) {
							if (message.toCharArray()[capcounter] == message
									.toLowerCase().toCharArray()[capcounter]) {// counts
																				// both
																				// upper
																				// case
																				// and
